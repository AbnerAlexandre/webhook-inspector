...
export function SectionDataTable({className, data, ...props}: SectionDataTableProps ) {
    ...
    return (
        <div className={twMerge('overflow-hiddne rounded-lg border border-zinc-700', className)} {...props}>
          <table className='w-full'>
            {data.map((item) => (
              <tr key={item.key} className='border-b border-zinc-700 last:border-0'>
                <td className='p-3 text-sm font-mediun text-zinc-400 bg-zinc-800/50 border-r border-zinc-700'>
                  {item.key}
                  </td>
